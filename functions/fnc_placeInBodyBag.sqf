...
if (_target isKindOf "CAManBase") then {
    if (_target isKindOf "OPTRE_Spartan2_Soldier_Base") exitWith {};
...
//Spartan body bag
if (_unit isKindOf "OPTRE_Spartan2_Soldier_Base") then {
    private _position = (getPosASL _patient) vectorAdd [0, 0, 0.2];
    ...
    // Create the body bag object, set its position to prevent it from flipping
    private _bodyBag = createVehicle ["ACE_bodyBagObject_Spartan", [0, 0, 0], [], 0, "NONE"];
    _bodyBag setPosASL _position;
    _bodyBag setDir _direction;
};
